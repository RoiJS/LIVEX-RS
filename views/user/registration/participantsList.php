<?php if(!empty($participantsList)){?>
	<?php foreach($participantsList as $participant){?>
		<tr>
			...
			<?php }else{?>
				<td><button class="btn btn-success" id="btn-register-participant<?php echo $participant["row"]["registrantID"];?>" onclick="registrationController.registerPreregisterParticipant(<?php echo $participant["row"]["registrantID"];?>);">Register</button></td>
			<?php }?>
			<td>
